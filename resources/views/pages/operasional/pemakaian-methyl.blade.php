    ...
    <div id="content-wrapper" class="d-flex flex-column">
      <div id="content">
        ...
        <div class="container-fluid" id="container-wrapper">
            ...
            <div class="modal-dialog">
              <div class="modal-content">
                ...
                <div class="modal-body">
                  <form method="POST" action="{{ route('Tambah Pemakaian Methyl') }}">
                    @csrf
                    <div class="mb-3">
                      <label for="id_pemakaian" class="form-label">ID Pemakaian:</label>
                      <input type="text" class="form-control" id="id_pemakaian" name="id_pemakaian"
                        value="PB00{{ $pemakaianMethyl->count() + 1 }}" required>
                    </div>
                    <div class="mb-3">
                    ...
                    <div class="mb-3">
                      ...
                      <input type="number" class="form-control" id="pemakaian_persediaan" name="pemakaian_persediaan"
                        required>
                    </div>
                    <div class="mb-3">
          ...
          @foreach ($pemakaianMethyl as $record)
          <!-- Modal Edit Data -->
          <div class="modal fade" id="editModal{{ $record->id }}" tabindex="-1" aria-labelledby="editModalLabel"
            ...
            <div class="modal-dialog">
              <div class="modal-content">
                ...
                <div class="modal-body">
                  <form method="POST" action="{{ route('Ubah Pemakaian Methyl', $record->id) }}">
                    @method('PUT')@csrf
                    <div class="mb-3">
                      <label for="id_pemakaian" class="form-label">ID Pemakaian:</label>
                      <input type="text" class="form-control" id="id_pemakaian" name="id_pemakaian"
                        value="{{ $record->id_pemakaian }}" required>
                    </div>
                    <div class="mb-3">
                      <label for="tanggal_mulai" class="form-label">Tanggal Mulai:</label>
                      <input type="datetime-local" class="form-control" id="tanggal_mulai" name="tanggal_mulai"
                        value="{{ $record->tanggal_mulai }}" required>
                    </div>
                    <div class="mb-3">
                      <label for="id_persediaan" class="form-label">ID Persediaan:</label>
                      <select class="form-control form-select-lg" name="id_persediaan" id="id_persediaan" required>
                    ...
                    <div class="mb-3">
                      <label for="pemakaian_persediaan" class="form-label">Pemakaian Persediaan:</label>
                      <input type="number" class="form-control" id="pemakaian_persediaan" name="pemakaian_persediaan"
                        value="{{ $record->pemakaian_persediaan }}" required>
                    </div>
                    <div class="mb-3">
                      <label for="keterangan" class="form-label">Keterangan:</label>
                      <input type="text" class="form-control" id="keterangan" name="keterangan"
          ...
          <div class="modal fade" id="deleteRecord{{ $record->id }}" tabindex="-1" aria-labelledby="deleteRecordLabel"
            ...
            <div class="modal-dialog modal-dialog-centered">
              <div class="modal-content">
                <form method="POST" action="{{ route('Hapus Pemakaian Methyl', $record->id) }}">
                  @method('delete')@csrf
                  <div class="modal-body">
                ...
                <div class="table-responsive p-3">
                  <table class="table align-items-center table-flush table-hover" id="dataTableHover">
                    ...
                    <tbody>
                      @foreach ($pemakaianMethyl as $record)
                      <tr>
                        <td>{{ $record->id_pemakaian }}</td>
                        <td>{{ $record->tanggal_mulai }}</td>
                        <td>{{ $record->dataPersediaan->id_persediaan }}</td>
                        <td>{{ $record->dataPersediaan->saldo }}</td>
                        <td>{{ $record->tanggal_selesai }}</td>
                        <td>{{ $record->pemakaian_persediaan }}</td>
                        <td>{{ $record->berat_akhir }}</td>
