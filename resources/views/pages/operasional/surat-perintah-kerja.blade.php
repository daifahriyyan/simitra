...
<head>
  ...
  <title>SIMITRA - Surat Perintah Kerja</title>
        ...
        <div class="container-fluid" id="container-wrapper">
            ...
            <div class="modal-dialog">
              <div class="modal-content">
                ...
                <div class="modal-body">
                  <form method="POST" action="{{ route('Tambah Surat Perintah Kerja') }}">
                    @csrf
                    <div class="mb-3">
                    ...
                    <div class="mb-3">
                      ...
                      <select class="form-control form-select-lg" name="fumigator" id="fumigator" required>
                        <option selected>Pilih Fumigator</option>
                    ...
                    <div class="mb-3">
                      ...
                      <select class="form-control form-select-lg" name="helper1" id="helper1" required>
                        <option selected>Pilih Helper 1</option>
                        @foreach ($dataPegawai as $item)
                        <option value="{{ $item->nama_pegawai }}">{{ $item->nama_pegawai }}</option>
                        @endforeach
                      </select>
                    ...
                    <div class="mb-3">
                      ...
                      <select class="form-control form-select-lg" name="helper2" id="helper2" required>
                        <option selected>Pilih Helper 2</option>
                        @foreach ($dataPegawai as $item)
                        <option value="{{ $item->nama_pegawai }}">{{ $item->nama_pegawai }}</option>
                        @endforeach
                      </select>
                    ...
                    <div class="mb-3">
                      ...
                      <select class="form-control form-select-lg" name="helper3" id="helper3" required>
                        <option selected>Pilih Helper 3</option>
                        @foreach ($dataPegawai as $item)
                        <option value="{{ $item->nama_pegawai }}">{{ $item->nama_pegawai }}</option>
                        @endforeach
                      </select>
          ...
          <div class="modal fade" id="editModal{{ $record->id }}" tabindex="-1" aria-labelledby="editModalLabel"
            ...
            <div class="modal-dialog">
              <div class="modal-content">
                ...
                <div class="modal-body">
                  <form method="POST" action="{{ route('Ubah Surat Perintah Kerja', $record->id) }}">
                    @csrf @method('put')
                    <div class="mb-3">
                      <label for="id_spk" class="form-label">ID SPK:</label>
                      <input type="text" class="form-control" id="id_spk" name="id_spk" value="{{ $record->id_spk }}"
                        readonly required>
                    </div>
                    <div class="mb-3">
                      <label for="tanggal" class="form-label">Tanggal:</label>
                    ...
                    <div class="mb-3">
                      <label for="dosis" class="form-label">Dosis (g/m³):</label>
                      <input type="number" class="form-control" id="dosis" name="dosis" value="{{ $record->dosis }}"
                        required>
                    </div>
                    <div class="mb-3">
                      <label for="fumigator" class="form-label">Fumigator:</label>
                      <select class="form-control form-select-lg" name="fumigator" id="fumigator" required>
                        <option selected>{{ $record->fumigator }}</option>
                        @foreach ($dataPegawai as $item)
                        <option value="{{ $item->nama_pegawai }}">{{ $item->nama_pegawai }}</option>
                        @endforeach
                      </select>
                    ...
                    <div class="mb-3">
                      <label for="helper1" class="form-label">Helper 1:</label>
                      <select class="form-control form-select-lg" name="helper1" id="helper1" required>
                        <option selected>{{ $record->helper1 }}</option>
                        @foreach ($dataPegawai as $item)
                        <option value="{{ $item->nama_pegawai }}">{{ $item->nama_pegawai }}</option>
                        @endforeach
                      </select>
                    ...
                    <div class="mb-3">
                      <label for="helper2" class="form-label">Helper 2:</label>
                      <select class="form-control form-select-lg" name="helper2" id="helper2" required>
                        <option selected>{{ $record->helper2 }}</option>
                        @foreach ($dataPegawai as $item)
                        <option value="{{ $item->nama_pegawai }}">{{ $item->nama_pegawai }}</option>
                        @endforeach
                      </select>
                    ...
                    <div class="mb-3">
                      <label for="helper3" class="form-label">Helper 3:</label>
                      <select class="form-control form-select-lg" name="helper3" id="helper3" required>
                        <option selected>{{ $record->helper3 }}</option>
                        @foreach ($dataPegawai as $item)
                        <option value="{{ $item->nama_pegawai }}">{{ $item->nama_pegawai }}</option>
                        @endforeach
                      </select>
          ...
          <div class="modal fade" id="deleteRecord{{ $record->id }}" tabindex="-1" aria-labelledby="deleteRecordLabel"
            ...
            <div class="modal-dialog modal-dialog-centered">
              <div class="modal-content">
                <form method="POST" action="{{ route('Hapus Surat Perintah Kerja', $record->id) }}">
                  @method('delete')@csrf
                  <div class="modal-body">
                ...
                <div class="card-header py-3 d-flex flex-row align-items-center justify-content-between">
                  ...
                  <div class="btn-group" role="group" aria-label="Basic mixed styles example">
                    ...
                    <div class="input-group">
                      <form action="{{ route('Surat Perintah Kerja') }}">
                    ...
                    <div>
                      <a href="{{ route('Surat Perintah Kerja') }}?export=pdf{{ (request()->tanggalMulai)? '&tanggalMulai='.request()->tanggalMulai : '' }}{{ (request()->tanggalAkhir)? '&tanggalAkhir='.request()->tanggalAkhir : '' }}"
