...
foreach ($invoices as $invoice) {
$idDataStandar = $invoice->id_data_standar;
        ...
        <div class="card-body">
          ...
          <div class="mb-3">
            <div class="small text-gray-500">Jenis Produk {{ $idDataStandar }}
              <div class="small float-right"><b>{{ $jumlah }} of 800 Items</b></div>
            </div>
            <div class="progress" style="height: 12px;">
              <div
                class="progress-bar bg-{{ ['warning', 'success', 'danger', 'info', 'success'][($idDataStandar - 1) % 5] }}"
                role="progressbar" style="width: {{ ($jumlah / 800) * 100 }}%"
                aria-valuenow="{{ ($jumlah / 800) * 100 }}" aria-valuemin="0" aria-valuemax="100"></div>
            </div>
          </div>
          @endforeach
        </div>
