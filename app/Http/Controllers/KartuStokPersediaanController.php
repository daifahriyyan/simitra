...
class KartuStokPersediaanController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $request['jumlah_keluar'] = intval($request['jumlah_keluar']);
        // Membuat Total Keluar
        $total_keluar = ($KPTerakhir == null) ? 1 : $KPTerakhir->harga_saldo * $request['jumlah_keluar'];
        // Membuat Jumlah Saldo
        $jumlah_saldo = ($KPTerakhir == null) ? 1 : $KPTerakhir->jumlah_saldo - $request['jumlah_keluar'];
        // Membuat Total Saldo
        $total_saldo = ($KPTerakhir == null) ? 1 : $KPTerakhir->total_saldo - $total_keluar;
        // Membaut Harga Saldo
        ...
        DataPersediaan::where('id', $request['id_persediaan'])->update([
            'saldo' => $dataPersediaan->saldo - ($KPTerakhir == null) ? 1 : $KPTerakhir->harga_saldo
        ]);
        ...
        KartuPersediaan::create([
            ...
            'jumlah_masuk' => 0,
            'total_masuk' => 0,
            'harga_keluar' => ($KPTerakhir == null) ? 1 : $KPTerakhir->harga_saldo,
